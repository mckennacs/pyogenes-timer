...
balJugTeam = new Array(
  "San D'oria vs Bastok",
  "Bastok vs Windurst",
  "San D'oria vs Windurst"
);
balPasTeam = new Array(
  "Bastok vs Windurst",
  "San D'oria vs Windurst",
  "San D'oria vs Bastok"
);
balMerTeam = new Array(
  "San D'oria vs Windurst",
  "San D'oria vs Bastok",
  "Bastok vs Windurst"
);
...
function getVanadielTime() {
  var now = new Date();
  vanaDate =
    (898 * 360 + 30) * msRealDay + (now.getTime() - basisDate.getTime()) * 25;
  ...
  VanaTime =
    "<DIV onmouseover='javascript:dayDetails(vDay)'><FONT COLOR=" +
    DayColor[vDay] +
    ">" +
    VanaDay[vDay] +
    "</FONT>:  ";
...
function dayDetails(firstDay) {
  ...
  out =
    out + '<TR><TH WIDTH=80 ALIGN="left">Day</TH><TH>Weak Element</TH></TR>';
  ...
  for (i = 0; i < 8; i++) {
    ...
    out =
      out +
      "<FONT COLOR=" +
      DayColor[(firstDay + i) % 8] +
      ">" +
      VanaDay[(firstDay + i) % 8] +
      "</FONT>";
  }
  ...
  for (i = 0; i < 8; i++) {
    ...
    out =
      out +
      "<FONT COLOR=" +
      weakColor[(firstDay + i) % 8] +
      ">" +
      weakMagic[(firstDay + i) % 8] +
      "</FONT>";
  }
...
function getConquest() {
  var now = new Date();
  var timeLeft =
    7 * msRealDay - ((now.getTime() - basisDate.getTime()) % (7 * msRealDay));
  ...
  if (conqDays < 10) {
    conqDays = "0" + Math.floor(conqDays);
  } else {
    conqDays = Math.floor(conqDays);
  }
  if (conqHours < 10) {
    conqHours = "0" + Math.floor(conqHours);
  } else {
    conqHours = Math.floor(conqHours);
  }
  if (conqMin < 10) {
    conqMin = "0" + Math.floor(conqMin);
  } else {
    conqMin = Math.floor(conqMin);
  }
  if (conqSec < 10) {
    conqSec = "0" + Math.floor(conqSec);
  } else {
    conqSec = Math.floor(conqSec);
  }
...
function getRSE() {
  ...
  if (race > 7) {
    RSECal =
      "<TABLE CLASS='blank' CELLPADDING='0' CELLSPACING='0'><TR><TH WIDTH='120' ALIGN='LEFT'>Date & Time</TH><TH WIDTH='90' ALIGN='LEFT'>Race</TH><TH WIDTH='90' ALIGN='LEFT'>Location</TH></TR>";
    for (i = 0; i < repeatCal; i++) {
      ...
      RSECal =
        RSECal +
        "<TR><TD>" +
        formatDate(RSEstart, 2) +
        "</TD><TD>" +
        RSErace[elapsedWeeks % 8] +
        "</TD><TD>";
      RSECal =
        RSECal +
        "<A HREF=#  onmousedown='javascript:getRSEDetails(" +
        (elapsedWeeks % 3) +
        ")'>";
    ...
    RSECal =
      "<TABLE CLASS='blank' CELLPADDING='0' CELLSPACING='0'><TR><TH WIDTH='120' ALIGN='LEFT'>Start</TH><TH WIDTH='120' ALIGN='LEFT'>End</TH><TH WIDTH='60' ALIGN='LEFT'>Location</TH></TR>";
    offsetTime = race * 8 * msGameDay;

    for (i = 0; i < repeatCal; i++) {
      elapsedWeeks = Math.floor((localtime - RSEtime) / (64 * msGameDay)) + i;

      elapsedLocationWeeks =
        Math.floor((localtime - RSEtime) / (8 * msGameDay)) + 8 * i;
...
function getRSEDetails(varLocation) {
  if (varLocation == 0) {
    output =
      "<TABLE CLASS='blank'><TR><TH ALIGN='left'>Key Droppers</TH><TH></TH><TH></TH></TR>";
    output =
      output +
      "<TR><TD VALIGN = 'TOP' WIDTH='75'>Sad Fly<BR>Gallinipper<BR>Rancid Ooze<BR>";
    output = output + "Banshee<BR>Mauthe Dog<BR>Myconid<BR>Wight</TD>";
    output =
      output + "<TD VALIGN = 'TOP' WIDTH='85'>Ghast<BR>Wendigo<BR>Spunkie<BR>";
    output = output + "Amphisbeana<BR>Thunder Elemental<BR>Greater Pugil</TD>";
    output = output + "</TR></TABLE>";
  } else if (varLocation == 1) {
    output =
      "<TABLE CLASS='blank'><TR><TH ALIGN='left'>Key Droppers</TH><TH></TH></TR>";
    output =
      output + "<TR><TD VALIGN = 'TOP'>Goblin Furrier<BR>Goblin Shaman<BR>";
    output =
      output +
      "Goblin Pathfinder<BR>Goblin Smithy<BR>Caterchipillar<BR>Wight<BR>";
    output =
      output +
      "<TD VALIGN = 'TOP' WIDTH='75'>Labyrinth Scorpion<BR>Abyss Worm<BR>Protozoan<BR>Air Elemental<BR>";
    ...
    output =
      output +
      "<TR><TD VALIGN = 'TOP' WIDTH='75'>Air Elemental<BR>Goblin Shaman<BR>Goblin Smithy<BR>";
...
function getMoonPhase() {
  ...
  nextPhase =
    "Next phase (" +
    PhaseName[(mnPhase + 1) % 8] +
    "): " +
    formatCountdown(toNextPhase);
  nextOptPhase =
    "Next " + PhaseName[optPhase] + ": " + formatCountdown(toOptimalPhase);

  mnpercent =
    "<DIV onmouseover='javascript:getMoonDetails()'>" + mnpercent + "</DIV>  ";

  document.getElementById("mPhase").innerHTML =
    mnpercent + nextPhase + "<BR>" + nextOptPhase;
  ...
  moonCal =
    "<TABLE CLASS='blank' CELLSPACING='0' CELLPADDING='0'><TR><TH WIDTH='95' ALIGN='left'>New Moon Start</TH><TH WIDTH='105' ALIGN='left'>New Moon End</TH><TH WIDTH='95' ALIGN='left'>Full Moon Start</TH><TH WIDTH='78' ALIGN='left'>Full Moon End</TH></TR>";
  for (i = 0; i < repeatCal; i++) {
    elapsedCycles =
      Math.floor((localTime - fullMoonBasis) / (84 * msGameDay)) + i;
    ...
    moonCal =
      moonCal +
      "<TR><TD>" +
      formatDate(NewStart, 2) +
      "</TD><TD>" +
      formatDate(NewEnd, 2) +
      "</TD><TD>" +
      formatDate(FullStart, 2) +
      "</TD><TD>" +
      formatDate(FullEnd, 2) +
      "</TD></TR>";
  }
  if (repeatCal < 1) {
    moonCal = "";
  } else {
    moonCal = moonCal + "</TABLE>";
  }
...
function getMoonDetails() {
  ...
  out =
    out +
    "<BR>Each lunar cycle takes exactly 84 game days (3 days, 8 hours, 38 minutes, 24 seconds)";
...
function getShipSched() {
  ...
  bSched =
    "<TABLE CLASS='blank' CELLSPACING='0' CELLPADDING='0'><TR><TH WIDTH='80' ALIGN='left'>Arrives</TH>";
  bSched = bSched + "<TH WIDTH='45'></TH><TH ALIGN='left' WIDTH='80'>ETA</TH>";
  bSched =
    bSched + "<TH WIDTH='80' ALIGN='left'>Departs</TH><TH WIDTH='45'></TH>";
  ...
  for (i = 0; i < repeatFerry; i++) {
    ...
    bSched =
      ...
      VanaDay[dPos2] +
      "</FONT></TD><TD>" +
      BoatSched2[(hours + i) % 3] +
      "</TD><TD>" +
      formatCountdown(arrivalTime) +
      '</TD><TD><FONT COLOR="' +
      DayColor[dPos] +
      '">' +
      VanaDay[dPos] +
...
function getAirSched() {
  ...
  vanaDate =
    (898 * 360 + 30) * msRealDay + (localTime - basisDate.getTime()) * 25;
  vDay = Math.floor((vanaDate % (8 * msRealDay)) / msRealDay);
...
function AirHelper(elapsed, offset, day, city) {
  ...
  output +=
    "<TD><FONT COLOR=" + DayColor[day] + ">" + VanaDay[day] + "</FONT></TD>";
...
function getDaySched() {
  ...
  var out =
    "<TABLE CLASS='blank' CELLSPACING='0' CELLPADDING='0'><TR><TH WIDTH='75' ALIGN='left'>Day</TH><TH WIDTH='100' ALIGN='left'>Begins</TH><TH ALIGN='left' WIDTH='100'>Ends</TH><TH ALIGN='left' WIDTH='120'>Moon Phase</TH></TR>";
  if (dayOffset > 7) {
    for (i = 0; i < repeatCal; i++) {
      ...
      out =
        out +
        "<TR><TD><FONT COLOR='" +
        DayColor[(dayOffset + i) % 8] +
        "'>" +
        VanaDay[(dayOffset + i) % 8] +
        "</FONT></TD><TD>" +
        formatDate(startTime, 2) +
        "</TD><TD>" +
        formatDate(endTime, 2) +
        "</TD><TD>" +
        mnpercent +
        "</TD></TR>";
    }
...
function getGuildHours() {
  ...
  guildOut =
    guildOut +
    "<TH WIDTH=55></TH><TH WIDTH=50></TH><TH ALIGN='left' WIDTH=140>Status</TH></TR>";
  guildOut =
    guildOut +
    "<TR><TD>" +
    "<A HREF=#  onmousedown='javascript:guildDetail(1)'>Alchemy</A>" +
    "</TD>" +
    alchemy +
    "</TR>";
  guildOut =
    guildOut +
    "<TR><TD>" +
    "<A HREF=#  onmousedown='javascript:guildDetail(2)'>Blacksmith</A>" +
    "</TD>" +
    blacksmith +
    "</TR>";
  guildOut =
    guildOut +
    "<TR><TD>" +
    "<A HREF=#  onmousedown='javascript:guildDetail(3)'>Boneworking</A>" +
    "</TD>" +
    bonework +
    "</TR>";
  guildOut =
    guildOut +
    "<TR><TD>" +
    "<A HREF=#  onmousedown='javascript:guildDetail(4)'>Goldsmith</A>" +
    "</TD>" +
    goldsmith +
    "</TR>";
  guildOut =
    guildOut +
    "<TR><TD>" +
    "<A HREF=#  onmousedown='javascript:guildDetail(5)'>Cloth</A>" +
    "</TD>" +
    cloth +
    "</TR>";
  guildOut =
    guildOut +
    "<TR><TD>" +
    "<A HREF=#  onmousedown='javascript:guildDetail(6)'>Woodworking</A>" +
    "</TD>" +
    wood +
    "</TR>";
  guildOut =
    guildOut +
    "<TR><TD>" +
    "<A HREF=#  onmousedown='javascript:guildDetail(7)'>Leather</A>" +
    "</TD>" +
    leather +
    "</TR>";
  guildOut =
    guildOut +
    "<TR><TD>" +
    "<A HREF=#  onmousedown='javascript:guildDetail(8)'>Fishing</A>" +
    "</TD>" +
    fishing +
    "</TR>";
  guildOut =
    guildOut +
    "<TR><TD>" +
    "<A HREF=#  onmousedown='javascript:guildDetail(9)'>Cooking</A>" +
    "</TD>" +
    cooking +
    "</TR>";
...
function guildHelper(open, close, holiday) {
  ...
  if (openTime >= localTime) {
    ...
    outputTxt2 =
      "</TD><TD><FONT COLOR='#DD0000'>Closed.</FONT> Open tomorrow.</TD>";
  ...
  if (holiday == vDay && closeTime > localTime) {
    guildCountdown = msGameDay - elapsedTime + open;
    outputTxt2 =
      "</TD><TD><FONT COLOR='#DD0000'>Closed</FONT> for Holiday</TD>";
    outputTxt1 = "<TD>Opens in:</TD><TD>";
  } else if (vDay + 1 == holiday && closeTime <= localTime) {
    guildCountdown = msGameDay - elapsedTime + open + msGameDay;
    outputTxt2 =
      "</TD><TD><FONT COLOR='#DD0000'>Closed.</FONT> Holiday tomorrow</TD>";
    outputTxt1 = "<TD>Opens in:</TD><TD>";
  }
...
function guildDetail(guild) {
  switch (guild) {
    case 1:
      out =
        "Alchemist´s guild <BR>Location:  Bastok Mines<BR>Holiday:  Lightsday<BR>Hours:  8:00-23:00";
      break;
    case 2:
      out =
        "Blacksmith´s guild <BR>Location:  Bastok Metalworks, Northern San d´Oria<BR>Holiday:  Watersday<BR>Hours:  8:00-23:00";
      break;
    case 3:
      out =
        "Boneworker´s guild <BR>Location:  Windurst Woods<BR>Holiday:  Windsday<BR>Hours:  8:00-23:00";
      break;
    case 4:
      out =
        "Goldsmith´s guild <BR>Location:  Bastok Market<BR>Holiday:  Iceday<BR>Hours:  8:00-23:00";
      break;
    case 5:
      out =
        "Weaver´s guild <BR>Location:  Windurst Woods<BR>Holiday:  Firesday<BR>Hours:  6:00-21:00";
      break;
    case 6:
      out =
        "Carpenter´s guild <BR>Location:   Northern San d´Oria<BR>Holiday:  Firesday<BR>Hours:  6:00-21:00";
      break;
    case 7:
      out =
        "Leatherworker´s guild <BR>Location:   Southern San d´Oria<BR>Holiday:  Iceday<BR>Hours:  3:00-18:00";
      break;
    case 8:
      out =
        "Fisherman´s guild <BR>Location:   Port Windurst<BR>Holiday:  Lightningday<BR>Hours:  3:00-18:00";
      break;
    case 9:
      out =
        "Culinary guild <BR>Location:   Windurst Waters<BR>Holiday:  Darksday<BR>Hours:  5:00-20:00";
  }
